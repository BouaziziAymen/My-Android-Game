...
public class HighPerformanceSpriteBatchVertexBufferObject extends HighPerformanceVertexBufferObject implements ISpriteBatchVertexBufferObject {
	...
	@Override
	public void addWithPackedColorLimited(ITextureRegion pTextureRegion, float pX1, float pY1, float pX2, float pY2, float pColorABGRPackedInt, float pLimitX0, float pLimitX1, float pLimitY0, float pLimitY1) {
		final float[] bufferData = this.getBufferData();
		final int bufferDataOffset = this.mBufferDataOffset;
		...
		final float deltaU = pTextureRegion.getU2() - pTextureRegion.getU();
		final float deltaV = pTextureRegion.getV2() - pTextureRegion.getV();

		final float u = pTextureRegion.getU()+pLimitX0/pTextureRegion.getWidth()*deltaU;
		final float v2 = pTextureRegion.getV2()-pLimitY0/pTextureRegion.getHeight()*deltaV;

		final float u2 = pTextureRegion.getU()+pLimitX1/pTextureRegion.getWidth()*deltaU;
		final float v = pTextureRegion.getV2()-pLimitY1/pTextureRegion.getHeight()*deltaV;
