...
public class Select {
	...
	public static Select instance () {
		if (Select.instance == null) Select.instance = new Select();
	...
	public <T> int selectIndex (T[] items, Comparator<T> comp, int kthLowest, int size) {
		if (size < 1) {
			throw new GdxRuntimeException("cannot show from empty array (size < 1)");
		...
		if (kthLowest == 1) {
			...
			if (this.quickSelect == null) this.quickSelect = new QuickSelect();
