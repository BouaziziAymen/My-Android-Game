...
public class Box2DSeparator {
     ...
     * @param fixtureDef  BlockA b2FixtureDef, containing all the properties (friction, density, etc.) which the new fixtures will inherit.
     ...
     */
    public static ArrayList<ArrayList<Vector2>>  separate(ArrayList<Vector2> verticesVec, float scale) {
    ...
    private static boolean isOnSegment(float px, float py, float x1, float y1, float x2, float y2) {
        boolean b1 = ((x1 + 1 >= px && px >= x2 - 1) || (x1 - 1 <= px && px <= x2 + 1));
        boolean b2 = ((y1 + 1 >= py && py >= y2 - 1) || (y1 - 1 <= py && py <= y2 + 1));
    ...
    private static void err() {
        throw new Error("BlockA problem has occurred. Use the Validate() method to see where the problem is.");
    }
}