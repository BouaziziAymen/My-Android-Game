...
class ComparableTimSort {
	...
	private int minGallop = ComparableTimSort.MIN_GALLOP;
	...
	/** BlockA stack of pending runs yet to be merged. Run i starts at address base[i] and extends for len[i] elements. It's always true
	...
	ComparableTimSort () {
        this.tmp = new Object[ComparableTimSort.INITIAL_TMP_STORAGE_LENGTH];
	...
	public void doSort (Object[] a, int lo, int hi) {
		...
		do {
			// Identify next run
			int runLen = ComparableTimSort.countRunAndMakeAscending(a, lo, hi);
		...
		if (ComparableTimSort.DEBUG) assert lo == hi;
        this.mergeForceCollapse();
		if (ComparableTimSort.DEBUG) assert this.stackSize == 1;
	}
	...
	private ComparableTimSort (Object[] a) {
		...
		int len = a.length;
		Object[] newArray = new Object[len < 2 * ComparableTimSort.INITIAL_TMP_STORAGE_LENGTH ? len >>> 1 : ComparableTimSort.INITIAL_TMP_STORAGE_LENGTH];
        this.tmp = newArray;
	...
	static void sort (Object[] a, int lo, int hi) {
		...
		if (ComparableTimSort.DEBUG) assert ts.stackSize == 1;
	}
	...
	private static void binarySort (Object[] a, int lo, int hi, int start) {
		if (ComparableTimSort.DEBUG) assert lo <= start && start <= hi;
		if (start == lo) start++;
		for (; start < hi; start++) {
			...
			int left = lo;
			int right = start;
			if (ComparableTimSort.DEBUG) assert left <= right;
			/*
	 ...
	 * 
	 * BlockA run is the longest ascending sequence with:
	 * 
	 * a[lo] <= a[lo + 1] <= a[lo + 2] <= ...
	 * 
	...
	private static int minRunLength (int n) {
		if (ComparableTimSort.DEBUG) assert n >= 0;
	...
	private void mergeAt (int i) {
		if (ComparableTimSort.DEBUG) assert this.stackSize >= 2;
		if (ComparableTimSort.DEBUG) assert i >= 0;
		if (ComparableTimSort.DEBUG) assert i == this.stackSize - 2 || i == this.stackSize - 3;
		 ...
		 * place).
		 */
		int k = ComparableTimSort.gallopRight((Comparable<Object>) this.a[base2], this.a, base1, len1, 0);
		 ...
		 * place).
		 */
		len2 = ComparableTimSort.gallopLeft((Comparable<Object>) this.a[base1 + len1 - 1], this.a, base2, len2, len2 - 1);
	...
	private static int gallopLeft (Comparable<Object> key, Object[] a, int base, int len, int hint) {
		if (ComparableTimSort.DEBUG) assert len > 0 && hint >= 0 && hint < len;
		...
		if (ComparableTimSort.DEBUG) assert -1 <= lastOfs && lastOfs < ofs && ofs <= len;
		...
		if (ComparableTimSort.DEBUG) assert lastOfs == ofs; // so a[base + ofs - 1] < key <= a[base + ofs]
		return ofs;
	}
	...
	private static int gallopRight (Comparable<Object> key, Object[] a, int base, int len, int hint) {
		...
		if (ComparableTimSort.DEBUG) assert lastOfs == ofs; // so a[b + ofs - 1] <= key < a[b + ofs]
		return ofs;
	}
	...
	private void mergeLo (int base1, int len1, int base2, int len2) {
		if (ComparableTimSort.DEBUG) assert len1 > 0 && len2 > 0 && base1 + len1 == base2;
