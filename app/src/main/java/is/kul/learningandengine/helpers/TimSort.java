...
/** BlockA stable, adaptive, iterative mergesort that requires far fewer than n lg(n) comparisons when running on partially sorted
...
class TimSort<T> {
	...
	private int minGallop = TimSort.MIN_GALLOP;
	...
	TimSort () {
        this.tmp = (T[])new Object[TimSort.INITIAL_TMP_STORAGE_LENGTH];
	...
	public void doSort (T[] a, Comparator<T> c, int lo, int hi) {
		...
		do {
			// Identify next run
			int runLen = TimSort.countRunAndMakeAscending(a, lo, hi, c);
		...
		if (TimSort.DEBUG) assert lo == hi;
        this.mergeForceCollapse();
		if (TimSort.DEBUG) assert this.stackSize == 1;
	...
	private TimSort (T[] a, Comparator<? super T> c) {
		...
		int len = a.length;
		T[] newArray = (T[])new Object[len < 2 * TimSort.INITIAL_TMP_STORAGE_LENGTH ? len >>> 1 : TimSort.INITIAL_TMP_STORAGE_LENGTH];
        this.tmp = newArray;
	...
	static <T> void sort (T[] a, int lo, int hi, Comparator<? super T> c) {
		...
		if (TimSort.DEBUG) assert ts.stackSize == 1;
	}
	...
	private static <T> void binarySort (T[] a, int lo, int hi, int start, Comparator<? super T> c) {
		if (TimSort.DEBUG) assert lo <= start && start <= hi;
		if (start == lo) start++;
		for (; start < hi; start++) {
			T pivot = a[start];
			...
			int left = lo;
			int right = start;
			if (TimSort.DEBUG) assert left <= right;
			/*
	...
	private void mergeAt (int i) {
		if (TimSort.DEBUG) assert this.stackSize >= 2;
		if (TimSort.DEBUG) assert i >= 0;
		if (TimSort.DEBUG) assert i == this.stackSize - 2 || i == this.stackSize - 3;
	...
	private static <T> int gallopLeft (T key, T[] a, int base, int len, int hint, Comparator<? super T> c) {
		if (TimSort.DEBUG) assert len > 0 && hint >= 0 && hint < len;
		int lastOfs = 0;
		int ofs = 1;
		...
		if (TimSort.DEBUG) assert -1 <= lastOfs && lastOfs < ofs && ofs <= len;
		...
		if (TimSort.DEBUG) assert lastOfs == ofs; // so a[base + ofs - 1] < key <= a[base + ofs]
		return ofs;
	}
	...
	private static <T> int gallopRight (T key, T[] a, int base, int len, int hint, Comparator<? super T> c) {
		...
		if (TimSort.DEBUG) assert lastOfs == ofs; // so a[b + ofs - 1] <= key < a[b + ofs]
		return ofs;
	}
	...
	private void mergeLo (int base1, int len1, int base2, int len2) {
		if (TimSort.DEBUG) assert len1 > 0 && len2 > 0 && base1 + len1 == base2;
