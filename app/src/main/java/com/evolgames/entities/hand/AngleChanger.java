...
public class AngleChanger {
    ...
    AngleChanger(GameEntity entity, float targetAngle){
        ...
        float dis = GeometryUtils.calculateShortestDirectedDistance(rot* MathUtils.radiansToDegrees,targetAngle);
    ...
    void run(){
        ...
        float rot = gameEntity.getBody().getAngle();
        float dis = GeometryUtils.calculateShortestDirectedDistance(rot* MathUtils.radiansToDegrees,target);
        if(Math.abs(dis)>0) {
