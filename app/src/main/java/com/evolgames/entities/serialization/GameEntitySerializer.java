...
public class GameEntitySerializer {
    ...
    transient public static Map<String,GameEntity> entities =  new HashMap<>();
    ...
    public GameEntity create() {
        ...
        return GameEntityFactory.getInstance().createGameEntity(initInfo.getX(), initInfo.getY(), initInfo.getAngle(),bodyInit,layerBlocks,bodyType,name);
    }
    ...
    private BodyInit getBodyInit() {
        ...
        if(initInfo.getMuzzleVelocity()!=null){
            bodyInit = new RecoilInit(bodyInit,null,initInfo.getRecoil(),initInfo.getMuzzleVelocity(),initInfo.getPoint());
        }
        return bodyInit;
    }
