...
public class ContactWrapper {
	...
	public ContactWrapper(Contact contact, int time, ContactImpulse impulse){
		...
		for(int i = 0; i< this.numberOfContactPoints; i++) this.points[i] =Vector2Pool.obtain( manifold.getPoints()[i]);
		//for(int i = 0; i< this.numberOfContactPoints; i++)Vector2Pool.recycle(manifold.getPoints()[i]);
	...
	public void setPoints(Vector2[] pts){
        this.points = new Vector2[this.numberOfContactPoints];
		for(int i = 0; i< this.numberOfContactPoints; i++) this.points[i] = Vector2Pool.obtain(pts[i]);
	}
