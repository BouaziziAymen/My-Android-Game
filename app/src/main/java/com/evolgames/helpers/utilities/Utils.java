...
public class Utils {
    ...
    public static void drawPath(ArrayList<Vector2> path, Color color) {
        for (int i = 0; i < path.size(); i++) {
            ...
            GameScene.plotter2.drawLine2(path.get(i), path.get(ni), color, 1);
        }
    }
    ...
    public static void translatePoints(ArrayList<Vector2> points, Vector2 center) {
        for (Vector2 v : points) v.sub(center);
    }
    ...
    public static Color getColorNonBlack(){
        ...
        return new Color(rand.nextFloat()*0.7f+0.3f,rand.nextFloat()*0.7f+0.3f,rand.nextFloat()*0.7f+0.3f);
    }

    public static void drawPathOnEntity(ArrayList<Vector2> path, Color color, Entity entity) {
        for(int i=0;i<path.size();i++){
            int ni = i==path.size()-1 ?0:i+1;
            Line line = new Line(path.get(i).x,path.get(i).y, path.get(ni).x,path.get(ni).y,5, ResourceManager.getInstance().vbom);
            line.setColor(color);
