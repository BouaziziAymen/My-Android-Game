...
public class FontLoader {
    ...
    // PhysicsConstants
    // ===========================================================
    private static final String LOADING_LETTERS_LIST = "01234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz.,_!?+-:èé";
    // ===========================================================
    // Fields
    // ===========================================================
    ...
    private SparseArray<ArrayList<MyLetter>> mLoadedFonts = new SparseArray<>();
    private SparseArray<Font> mFonts = new SparseArray<>();
    ...
    public void loadFont(int pFontId, final Typeface pTypeface, final float pSize, final int pColorARGBPackedInt, final boolean pAntiAlias) {
    ...
    private MyLetter createLetter(char character) throws FontException {
        ...
        this.mCanvas.drawRect(0, 0, letterBitmap.getWidth(), letterBitmap.getHeight(), this.mBackgroundPaint);

        /* Actually draw the character. */
        this.drawLetter(characterAsString, 1, 1 - rect.top);

        TextureRegion letterTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromSource(mTexture, new BitmapTextureAtlasSource(letterBitmap));
    ...
    public MyLetter getLetter(int fontId, char character) {
        int index = (Character.isWhitespace(character)) ? 0 : LOADING_LETTERS_LIST.indexOf(character) + 1;
