...
public class VerticesFactory {
    ...
    public static ArrayList<Vector2> createRectangle(float x,float y,float wleft,float wright, float hup,float hdown) {
    ...
    public static ArrayList<Vector2> createPolygon(float x, float y, float angle, float w, float h, int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            list.add(Vector2Pool.obtain(x + w * (float) Math.cos(angle), y + h * (float) Math.sin(angle)));
            angle += 2 * Math.PI / n;
