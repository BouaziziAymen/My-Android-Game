...
public class SecondaryButtonField extends SecondaryLinearLayout {
    ...
    public SecondaryButtonField(int primaryKey, int secondaryKey, OneLevelGameWindowController controller) {
        super(primaryKey, secondaryKey, 4);
        setWindowPartIdentifier(WindowPartIdentifier.WINDOW_BODY);
        buttonControl = new ButtonWithText<>("", 2, ResourceManager.getInstance().largeLampTextureRegion, Button.ButtonType.Selector, true);
        addToLayout(buttonControl);

        buttonControl.setBehavior(new ButtonBehavior<OneLevelGameWindowController>(controller, buttonControl) {
            @Override
