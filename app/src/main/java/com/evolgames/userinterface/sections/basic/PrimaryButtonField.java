...
public class PrimaryButtonField extends PrimaryLinearLayout {
    ...
    public PrimaryButtonField(int primaryKey, OneLevelGameWindowController controller) {
        ...
        controlButton = new ButtonWithText<>("", 2, ResourceManager.getInstance().mainButtonTextureRegion, Button.ButtonType.Selector,true);
        addToLayout(controlButton);


        controlButton.setBehavior(new ButtonBehavior<OneLevelGameWindowController>(controller,controlButton) {
            @Override
