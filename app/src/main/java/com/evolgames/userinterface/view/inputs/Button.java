...
public class Button<C extends Controller> extends ClickableImage<C,ButtonBehavior<C>> {
    ...
    public Button(TiledTextureRegion tiledTextureRegion, ButtonType buttonType,boolean createBounds) {
        super(tiledTextureRegion,createBounds);
        this.buttonType = buttonType;
        this.tiledTextureRegion = tiledTextureRegion;
    }

    public Button(float pX, float pY, TiledTextureRegion tiledTextureRegion, ButtonType buttonType, boolean createBounds) {
    ...
    public void updateTileIndex(State state){
        switch (state) {
            ...
            case PRESSED:
                ...
                break;
            case DISABLED:
                if (tiledTextureRegion.getTileCount() >= 3)
                    tileIndex = 2;
                break;
        }
    }
