...
public abstract class Controller extends HUD implements IOnSceneTouchListener {
  ...
  public Controller(
      ...
      final Controller.IOnScreenControlListener pOnScreenControlListener) {
    ...
    this.mControlBase =
        new Sprite(
            pX,
            pY,
            ResourceManager.getInstance().controlButton,
          ...
          public boolean onAreaTouched(
            ...
            return Controller.this.onHandleControlBaseTouched(
    ...
    this.registerUpdateHandler(
        new TimerHandler(
            ...
            true,
            pTimerHandler ->
                Controller.this.mOnScreenControlListener.onControlChange(
                    Controller.this,
                    Controller.this.mControlValueX,
                    Controller.this.mControlValueY)));
  ...
  private void updateControlKnob(final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
    ...
    final float relativeX =
        (MathUtils.bringToBounds(0, controlBase.getWidth(), pTouchAreaLocalX)
                / controlBase.getWidth())
            - 0.5f;
    final float relativeY =
        (MathUtils.bringToBounds(0, controlBase.getHeight(), pTouchAreaLocalY)
                / controlBase.getHeight())
            - 0.5f;
  ...
  public interface IOnScreenControlListener {
    void onControlChange(
        final Controller pBaseOnScreenControl, final float pValueX, final float pValueY);
  }
}
