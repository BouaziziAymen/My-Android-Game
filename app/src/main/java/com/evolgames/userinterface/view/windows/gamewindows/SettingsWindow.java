...
public abstract class SettingsWindow  extends AbstractThreeLevelSectionedAdvancedWindow<SimplePrimary<?>, SimpleSecondary<?>, SimpleTertiary<?>, SimpleQuaternary<?>> {
    ...
    public void addSecondary(SimpleSecondary<?> secondaryElement){
        getLayout().addSecondary(secondaryElement.getPrimaryKey(),secondaryElement.getSecondaryKey(), secondaryElement);
    }
    public void addTertiary(SimpleTertiary<?> tertiaryElement){
        getLayout().addTertiary(tertiaryElement.getPrimaryKey(),tertiaryElement.getSecondaryKey(),tertiaryElement.getTertiaryKey(), tertiaryElement);
    }

    public void addQuaternary(SimpleQuaternary<?> quaternaryElement){
        getLayout().addQuaternary(quaternaryElement.getPrimaryKey(),quaternaryElement.getSecondaryKey(),quaternaryElement.getTertiaryKey(),quaternaryElement.getQuaternaryKey(), quaternaryElement);
    }

    @Override
    protected ThreeLevelSectionLayout<SimplePrimary<?>, SimpleSecondary<?>,SimpleTertiary<?>,SimpleQuaternary<?>> createLayout() {
        return new ThreeLevelSectionLayout<>(8,getLocalVisibilitySup(), LinearLayout.Direction.Vertical);
    }
    ...
    @Override
    public SimpleQuaternary<?> createQuaternary(int primaryKey, int secondaryKey, int tertiaryKey, int quaternaryKey) {
        throw new UnsupportedOperationException();
    }
}
