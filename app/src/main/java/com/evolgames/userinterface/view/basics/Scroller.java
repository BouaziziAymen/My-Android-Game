...
public class Scroller extends LinearLayout implements Touchable {
    ...
    private final static float UPPER_HEIGHT = 27f;
    private final static float SPACE_MARGIN = -1f;
    private final static float SLOT_WIDTH = 24f;
    private final static float TRAIL_WIDTH = 10f;
    ...
    public Scroller(float pX, float pY, int rows, LinearLayoutAdvancedWindowController controller, float visibilityLength) {
        ...
        for (int i = 0; i < rows; i++) {
            Image middle = new Image(-TRAIL_WIDTH / 2, 0, ResourceManager.getInstance().trailTextureRegion);
            addToLayout(middle);
        }
