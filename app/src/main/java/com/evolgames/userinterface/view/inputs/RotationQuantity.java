...
public class RotationQuantity<C extends AdvancedWindowController<?>> extends Quantity<C> {
    ...
    private void buildButtons(C controller) {
        anticlockwiseButton = new Button<>(getWidth() - 8 - inputLength, 12 - 7, ResourceManager.getInstance().rotationAntiClockTextureRegion, Button.ButtonType.Selector, true);
        addElement(anticlockwiseButton);
        anticlockwiseButton.setBehavior(new ButtonBehavior<C>(controller, anticlockwiseButton) {
            @Override
        ...
        clockwiseButton = new Button<>(12 - 8, 12 - 7, ResourceManager.getInstance().rotationClockTextureRegion, Button.ButtonType.Selector, true);
        addElement(clockwiseButton);


        clockwiseButton.setBehavior(new ButtonBehavior<C>(controller, clockwiseButton) {
            @Override
            public void informControllerButtonClicked() {
