...
public class Switcher extends Container implements Temporal {
    ...
    public Switcher(float x, float y, List<TextureRegion> regions, float imageWidth, IntConsumer onIndexChanged) {
        ...
        right = new Button<>(ResourceManager.getInstance().switcherRight, Button.ButtonType.OneClick, false);
        right.setBounds(new CircularBounds(right, 15f));
        right.setBehavior(new ButtonBehavior<Controller>(controller, right) {
            @Override
        ...
        left = new Button<>(ResourceManager.getInstance().switcherLeft, Button.ButtonType.OneClick, false);
        left.setBounds(new CircularBounds(left, 15f));
        left.setBehavior(new ButtonBehavior<Controller>(controller, left) {
            @Override
            public void informControllerButtonClicked() {
                moveBackward();
            }
