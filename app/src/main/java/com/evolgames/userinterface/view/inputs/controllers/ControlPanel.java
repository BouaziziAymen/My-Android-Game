...
public class ControlPanel {
  ...
  public ControlElement allocateController(
    ...
    ControlElement element =
        new ControlElement(
            ControlElement.Type.AnalogController, controlIdCounter.getAndIncrement(), action);
    ...
    if (!registeredElements.contains(e)) {
      switch (e.getType()) {
        case AnalogController:
          {
            AnalogController control =
                new AnalogController(x, y, this.camera, 0.1f, createAnalogListener());
        ...
        case DigitalController:
          {
            DigitalController control =
                new DigitalController(x, y, this.camera, 0.1f, false, createBaseListener());
            control.setUserData(e);
            e.setAssociate(control);
            registeredElements.add(e);
            scene.setChildScene(control);
  ...
  private AnalogController.IAnalogOnScreenControlListener createAnalogListener() {
    ...
    return new AnalogController.IAnalogOnScreenControlListener() {
      ...
      @Override
      public void onControlChange(Controller pBaseOnScreenControl, float pValueX, float pValueY) {
