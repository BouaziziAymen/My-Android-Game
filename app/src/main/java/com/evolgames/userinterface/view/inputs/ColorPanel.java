...
public class ColorPanel extends Panel {
    ...
    public ColorPanel(float pX, float pY, ColorSelectorWindowController controller) {
        ...
        float deltax = (effectiveLength - NUMBER_SQUARES * SQUARE_SIDE - (NUMBER_SQUARES - 1) * MARGIN) / 2f;
        colorsLayout = new BoxLayout(55 + deltax, BLOCK_HEIGHT  - SQUARE_SIDE -MARGIN-9, MARGIN, 2, 8);
    ...
    public void addColor(final float pRed, final float pGreen, final float pBlue) {
        ...
        Button<ColorSelectorWindowController> square = new Button<>(ResourceManager.getInstance().squareTextureRegion, Button.ButtonType.Selector, true);
        colorsLayout.addToLayout(square);
        square.setColor(pRed, pGreen, pBlue);
        square.setBehavior(new ButtonBehavior<ColorSelectorWindowController>(mController,square) {
            @Override
