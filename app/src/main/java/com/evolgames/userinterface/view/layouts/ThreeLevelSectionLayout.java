...
public class ThreeLevelSectionLayout<Primary extends Element,Secondary extends Element,Tertiary extends Element,Quaternary extends Element> extends
        SectionLayout<ThreeLevelSection<Primary,Secondary,Tertiary,Quaternary>, Primary>
{
    private float[] margins = new float[]{8,24,40,56};
    public ThreeLevelSectionLayout(float pX, float pY, Direction direction) {
        super(pX, pY, direction);
    }

    public ThreeLevelSection<Primary,Secondary,Tertiary,Quaternary> getSectionByKey(int sectionKey){
        for(ThreeLevelSection<Primary,Secondary,Tertiary,Quaternary> section:sectionsList)if(sectionKey==section.getSectionKey())return section;
        return null;
    }
    ...
    public Tertiary getTertiaryByIndex(int primaryKey, int secondaryKey,int index){
        return getSectionByKey(primaryKey).getSectionByKey(secondaryKey).getChildren().get(index).getPrimary();
    }

    public Quaternary getQuaternaryByIndex(int primaryKey, int secondaryKey,int tertiaryKey, int index){
        return getSectionByKey(primaryKey).getSectionByKey(secondaryKey).getSectionByKey(tertiaryKey).getChildren().get(index).getPrimary();
    }
    ...
    public int getQuaternatiesSize(int primaryKey, int secondaryKey,int tertiaryKey) {
        return getSectionByKey(primaryKey).getSectionByKey(secondaryKey).getSectionByKey(tertiaryKey).getChildren().size();
    }
    ...
    public Tertiary getTertiary(int primaryKey, int secondaryKey,int tertiaryKey){
        return getSectionByKey(primaryKey).getSectionByKey(secondaryKey).getSectionByKey(tertiaryKey).getPrimary();
    }
    public Quaternary getQuaternary(int primaryKey, int secondaryKey,int tertiaryKey,int quaternaryKey){
        return getSectionByKey(primaryKey).getSectionByKey(secondaryKey).getSectionByKey(tertiaryKey).getSectionByKey(quaternaryKey).getPrimary();
    }

    @Override
    protected ThreeLevelSection<Primary, Secondary,Tertiary,Quaternary> createSection(int key,Primary p, boolean isActive) {
        return new ThreeLevelSection<>(key,p,isActive);
    }

    @Override
    protected ThreeLevelSection<Primary, Secondary, Tertiary,Quaternary> createDummySection(Element e) {
        return new ThreeLevelSection<>(e);
    }

    public void addSecondary(int primaryKey, int secondaryKey, Secondary s,boolean isActive) {
        ThreeLevelSection<Primary, Secondary, Tertiary,Quaternary> section = getSectionByKey(primaryKey);
    ...
    public void addQuaternary(int primaryKey, int secondaryKey,int tertiaryKey,int quaternaryKey, Quaternary q) {
        ThreeLevelSection<Primary, Secondary, Tertiary,Quaternary> section = getSectionByKey(primaryKey);
    ...
    public void updateLayout(){
        ...
        for(int i = 0; i< sectionsList.size(); i++){
            ...
            ArrayList<TwoLevelSection<Secondary, Tertiary,Quaternary>> secondarySections = section.getChildren();
            for(int j=0;j<secondarySections.size();j++){
                TwoLevelSection<Secondary, Tertiary,Quaternary> secondarySection = secondarySections.get(j);
                ...
                ArrayList<OneLevelSection<Tertiary,Quaternary>> tertiarySections = secondarySection.getChildren();
                for(int k=0;k<tertiarySections.size();k++){
                    ...
                    ArrayList<ZeroLevelSection<Quaternary>> quaternarySections = tertiarySection.getChildren();
    ...
    public void removeQuaternary(int primaryKey, int secondaryKey, int tertiaryKey,int quaternaryKey) {
        getSectionByKey(primaryKey).getSectionByKey(secondaryKey).getSectionByKey(tertiaryKey).removeSection(quaternaryKey);
    }
