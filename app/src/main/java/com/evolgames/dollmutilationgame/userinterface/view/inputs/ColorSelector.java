...
import com.evolgames.dollmutilationgame.userinterface.control.behaviors.ColorSelectorBehavior;
...
public class ColorSelector
    ...
    public ColorSelector(float pX, float pY, ColorSelectorWindowController controller) {
        ...
        this.mesh =
                new Mesh(
                        0, 0, circle_vertices, 24, DrawMode.TRIANGLE_FAN, ResourceManager.getInstance().vbom);

        this.value = 100;
        this.alpha = 1f;
        setBehavior(new ColorSelectorBehavior(controller, this));

        Button<LinearLayoutAdvancedWindowController<?>> addColorButton =
                new Button<>(ResourceManager.getInstance().add1, Button.ButtonType.OneClick, true);
        addColorButton.setPosition(
                17 - addColorButton.getWidth() / 2, 33 - addColorButton.getHeight() / 2);
        addColorButton.setBehavior(
        ...
        removeColorButton =
                new Button<>(ResourceManager.getInstance().minus1, Button.ButtonType.OneClick, true);
        removeColorButton.setPosition(
                33 - removeColorButton.getWidth() / 2, 17 - removeColorButton.getHeight() / 2);
        removeColorButton.setBehavior(
                new ButtonBehavior<ColorSelectorWindowController>(controller, removeColorButton) {
                    @Override
                    public void informControllerButtonClicked() {
    ...
    public void updateColorRGB() {
        ...
        Color.RGBToHSV(
                (int) Math.floor(red * 255),
                (int) Math.floor(green * 255),
