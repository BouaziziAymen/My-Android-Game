...
import com.evolgames.dollmutilationgame.userinterface.view.windows.AdvancedWindow;
...
public class AdvancedWindowController<W extends AdvancedWindow<?>> extends Controller {
    ...
    protected void hideWindowBody() {
        for (Element e : window.getContents()) {
            if (isElementPartOf(e, WindowPartIdentifier.WINDOW_BODY)
                    || isElementPartOf(e, WindowPartIdentifier.SCROLLER)
                    || isElementPartOf(e, WindowPartIdentifier.WINDOW_MID_PADDING)) {
                e.setVisible(false);
            }
        }
    }
    ...
    protected void onTextFieldTapped(TextField<?> pTextField) {
        ...
        keyboardController.openKeyboard(
                pTextField.getAbsoluteX(),
                pTextField.getAbsoluteY(),
                selectedTextField.getWidth(),
                selectedTextField.getHeight());
    }
