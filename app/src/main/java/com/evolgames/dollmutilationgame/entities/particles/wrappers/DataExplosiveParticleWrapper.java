...
import com.evolgames.dollmutilationgame.entities.particles.initializers.RandomVelocityInitializer;
import com.evolgames.dollmutilationgame.entities.particles.modifiers.AlphaParticleModifier;
import com.evolgames.dollmutilationgame.entities.particles.modifiers.GroundCollisionBumpModifier;
import com.evolgames.dollmutilationgame.entities.particles.modifiers.GroundCollisionExpireModifier;
...
public class DataExplosiveParticleWrapper extends ExplosiveParticleWrapper {
    public DataExplosiveParticleWrapper(GameEntity gameEntity, float[] data, float velocity, float fireRatio, float smokeRatio, float sparkRatio, float particles, float flameTemp, float initialFlameParticleSize, float finalFlameParticleSize) {
        super(gameEntity, velocity, fireRatio, smokeRatio, sparkRatio, particles, flameTemp, initialFlameParticleSize, finalFlameParticleSize, data);
        createSystems();
    }
    ...
    protected BaseParticleSystem createFireSystem(int lowerRate, int higherRate, int maxParticles, float verticalSpeed, float horizontalSpeed, float initialFlameParticleSize, float finalFlameParticleSize) {
        ...
        float lifeSpan = 0.5f;
        this.fireParticleSystem.addParticleModifier(new org.andengine.entity.particle.modifier.ColorParticleModifier<>(0f, lifeSpan, fireColor.getRed(), secondColor.getRed(), fireColor.getGreen(), secondColor.getGreen(), fireColor.getBlue(), secondColor.getBlue()));
        this.fireParticleSystem.addParticleInitializer(new ColorParticleInitializer<>(fireColor));
        this.fireParticleSystem.addParticleModifier(new AlphaParticleModifier<>(0f, lifeSpan, 0.3f, 0f));
        this.fireParticleSystem.addParticleModifier(new ScaleParticleModifier<>(0f, lifeSpan, Math.min(0.99f, initialFlameParticleSize), Math.min(0.99f, finalFlameParticleSize)));
