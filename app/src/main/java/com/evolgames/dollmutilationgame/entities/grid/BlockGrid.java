package com.evolgames.dollmutilationgame.entities.grid;
...
public class BlockGrid {
    ...
    public CoatingBlock getCoatingBlock(int nx, int ny) {
        for (CoatingBlock g : coatingBlocks) if (g.getNx() == nx && g.getNy() == ny) return g;
        return null;
    }
    ...
    public HashSet<CoatingBlock> findNeighbors(CoatingBlock grain) {
        ...
        CoatingBlock temp;
        if ((temp = getCoatingBlock(grain.getNx() - 1, grain.getNy() - 1)) != null)
            neighbors.add(temp);
