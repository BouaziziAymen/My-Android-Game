...
public class RagDoll extends GameGroup {
    ...
    private void detectGround() {
        ...
        if (upperTorso != null && upperTorso.getBody() != null) {
            ...
            torsoReadyToStand = Math.abs(GeometryUtils.normalizeAngleRad(upperTorsoBody.getAngle())) < Math.PI / 6;
        }
        if (upperLegR != null && upperLegR.getBody() != null) {
            Body upperLegRBody = upperLegR.getBody();
            rightUpperLegStraight = Math.abs(GeometryUtils.normalizeAngleRad(upperLegRBody.getAngle())) < Math.PI / 6;
        }

        if (upperLegL != null && upperLegL.getBody() != null) {
            Body upperLegLBody = upperLegL.getBody();
            leftUpperLegStraight = Math.abs(GeometryUtils.normalizeAngleRad(upperLegLBody.getAngle())) < Math.PI / 6;
        }
