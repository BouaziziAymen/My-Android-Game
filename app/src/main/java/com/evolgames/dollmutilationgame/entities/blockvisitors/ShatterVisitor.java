...
public class ShatterVisitor extends BreakVisitor<LayerBlock> {
    ...
    private void processBlock(LayerBlock layerBlock, boolean isFirst) {
        ...
        if (layerBlock.getBlockArea() < 10 * PhysicsConstants.MINIMUM_STABLE_SPLINTER_AREA && availableImpulse > (pulverizationImpulse = calculatePulverizationImpulse(layerBlock))) {
            ...
            if (availableImpulse - data.getDestructionEnergy() >= 0) {
                ...
                if (!data.isNonValid() && (isFirst || layerBlock.getBlockArea() >= 5 * PhysicsConstants.MINIMUM_STABLE_SPLINTER_AREA)) {
                    shatterPerformed = true;
