...
public class MeshBatch extends Shape {
    ...
    public MeshBatch(final int pCapacity, final VertexBufferObjectManager pVertexBufferObjectManager) {
        this(pCapacity, pVertexBufferObjectManager, DrawType.STATIC);
    }



    public MeshBatch(final int pCapacity, final VertexBufferObjectManager pVertexBufferObjectManager, final DrawType pDrawType) {
        this( pCapacity, new HighPerformanceMeshBatchVertexBufferObject(pVertexBufferObjectManager, pCapacity, pDrawType, true, MeshBatch.VERTEXBUFFEROBJECTATTRIBUTES_DEFAULT));
    }
    ...
    public MeshBatch(final int pCapacity,  HighPerformanceMeshBatchVertexBufferObject pSpriteBatchVertexBufferObject) {
        this(pCapacity, pSpriteBatchVertexBufferObject, PositionColorShaderProgram.getInstance());
    }


    public MeshBatch( final int pCapacity,  HighPerformanceMeshBatchVertexBufferObject pSpriteBatchVertexBufferObject, final ShaderProgram pShaderProgram) {
        this(0, 0, pCapacity, pSpriteBatchVertexBufferObject, pShaderProgram);
    }

    public MeshBatch(final float pX, final float pY, final int pCapacity,  HighPerformanceMeshBatchVertexBufferObject pSpriteBatchVertexBufferObject, final ShaderProgram pShaderProgram) {
...
//		GLState.enableDepthMask(pGL);
    ...
    private void addInner(float x1,float y1, float x2,float y2, float x3, float y3) {
        ...
        float pBlue = 1;
        float pAlpha = 1;
        this.mMeshBatchVertexBufferObject.addWithPackedColor( x1,y1,x2,y2,x3,y3, ColorUtils.convertRGBAToABGRPackedFloat(pRed, pGreen, pBlue, pAlpha));
    }
